...
#show: main-zh.with(
  ...
  draft: false,
  tags: (
    blog-tags.temporal-antialiasing,
    blog-tags.rendering,
  ),
)
...
#figure(
  image("./images/motion-vector-before.png", width: 80%),
  caption: "物体边缘的 pixel - previous frame"
) <fig-pixel-history>

#figure(
  image("./images/motion-vector-after.png", width: 80%),
  caption: "物体边缘的 pixel - current frame"
)

如 @fig-pixel-history 及其后一张图所示，蓝色表示物体，白色表背景颜色。每个小矩形代表一个pixel，其中含有四个sample。包围在圆中的sample为当前帧所采样的sample。前一张图为前一帧的情况，后一张图为当前帧的情况。物体相对于前一帧而言向右运动了一段距离。

在当前帧中，对后一张图中被标记为红色的sample进行分析。该sample所属的pixel位于物体的左边缘，且该红色的sample没有被物体边缘覆盖，所以该sample的motion vector为背景的motion vector。假设背景没有运动，则该motion vector为0，从而导致该sample的history color为前一张图中的绿色sample的颜色，即背景颜色。则在当前帧中，该pixel的颜色为背景颜色。然而，在当前帧中，如果我们需要物体边缘的pixel具有反走样的效果，其history color应该使用前一张图中的红色sample位置的历史颜色。通过将前一张图中红色sample位置的历史颜色和后一张图中红色sample的当前颜色进行blending后才能够得到反走样效果。
