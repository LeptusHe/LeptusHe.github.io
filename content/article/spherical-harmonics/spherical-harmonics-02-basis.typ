...
#show: main-zh.with(
  title: "球谐函数02 - 基础理论",
  desc: [介绍复数与实数球谐函数的定义、正交性和基函数构成，说明伴随勒让德多项式、带状与扇形球谐函数的关系，以及它们在球面函数表达中的作用。],
  date: "2023-07-09",
  draft: false,
  tags: (
    blog-tags.spherical-harmonics,
    blog-tags.rendering,
...
$ f(theta, phi) = sum_(l >= 0)^(infinity) sum_(m = -l)^(l) c_l^m S_l^m (theta, phi) $ <spherical-harmonics-fitting>

其中$S_l^m (theta, phi)$为球谐函数，定义如 @spherical-harmonics 所示，其中$l, m in bb(N)$， 且满足$-l <= m <= l$。$P_l^m (x)$为 _伴随勒让德多项式_。
