...
export default defineConfig({
  ...
  integrations: [
    sitemap({
      filter: (page) => !new URL(page).pathname.match(/^\/article\/.+\/$/),
    }),
    typst({
    ...
    tylant({
      ...
      components: {
        CommentList: resolve("./src/components/DisabledCommentList.astro"),
        minimal: {
          BaseHead: resolve("./src/components/BaseHead.astro"),
          Header: resolve("./src/components/Header.astro"),
          Footer: resolve("./src/components/Footer.astro"),
        },
      },
    }),
  ],
